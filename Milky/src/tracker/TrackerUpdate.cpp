...
bool Tracker::updateGuitarControl(GuitarControl* guitarControl)
{
	if (!guitarControl)
		return false;

	if (!guitarControl->isVisible())
		return false;

	pp_int32 ins = listBoxInstruments->getSelectedIndex() + 1;

	pp_int32 i, mn = ModuleEditor::MAX_NOTE, nn = 0, chan = guitarControl->channel;

	bool tab[mn] ;

	for (i = 0; i < mn; i++) // hide all notes
	{
		tab[i] = guitarControl->getNoteState(i);
		guitarControl->pressNote(i, false);     //, false, false
	}

	if(chan)
	{
		for (i = chan-1; i < chan + 6 ; i++)
		{
			pp_int32 note;
			bool muted = false;
			if (playerController->isNotePlaying(ins, i, note, muted))
			{
				guitarControl->pressNote(note - 1, true, muted, i+1);
			}
		}
	}
	else
	{
		for (i = 0; i < playerController->getPlayerNumPlayingChannels(); i++)
		{
			...
			if (playerController->isNotePlaying(ins, i, note, muted))
			{
				guitarControl->pressNote(note - 1, true, muted, i);
			}
		}
	}
	bool update = false;
	for (i = 0; i < mn; i++)
	{
		if (guitarControl->getNoteState(i) != tab[i])
		{
			if(tab[i] == false)  // new note on
			{
				guitarControl->rangNote(i, 1);
				nn++;
			}
			else    // note off
			{
				guitarControl->rangNote(i, 0);  
				if(nn) nn--;
			}
			update = true;
		//	break;
	...
	if (update)
	{
		//  new rang for old on-keys
		for (i = 0; i < mn; i++)
		{
			if(tab[i] == true)
				guitarControl->rangNote(i, nn);
		}
...
void Tracker::updateJgitMenuOrder(bool repaint/* = true*/)
{
	PPContainer* container = static_cast<PPContainer*>(screen->getControlByID(CONTAINER_LOWRES_JGITMENU));
	PPStaticText* staticText = static_cast<PPStaticText*>(container->getControlByID(STATICTEXT_JGITMENU_CURORDER));
	staticText->setHexValue(getOrderListBoxIndex(), 2);
	if (container->isVisible())
		screen->paintControl(container, repaint);
}
 ...
 */
		...
		if (container->isVisible())
		{
			staticText = static_cast<PPStaticText*>(container->getControlByID(STATICTEXT_JGITMENU_CURPATTERN));
			staticText->setHexValue(moduleEditor->getCurrentPatternIndex(), 2);	
			screen->paintControl(container, repaint);
		}   
	}
...
/*
		container = static_cast<PPContainer*>(screen->getControlByID(CONTAINER_LOWRES_JGITMENU));
		staticText = static_cast<PPStaticText*>(container->getControlByID(STATICTEXT_JGITMENU_CURINSTRUMENT));
		staticText->setHexValue(i+1, 2);	
		if (container->isVisible())
		{	screen->paintControl(container, repaint);
		}
...
void Tracker::doFollowSong()
{
	...
	//td   	
	updateRecordButton(static_cast<PPContainer*>(screen->getControlByID(CONTAINER_LOWRES_JGITMENU)), pColor);
#endif
	}
	
	// check if the piano has been updated
	bool updatePiano = updatePianoControl(sectionInstruments->getPianoControl());
//	bool updateGuitar = updateGuitarControl(sectionInstruments->getGuitarControl());
	...
	else if (screen->getControlByID(CONTAINER_LOWRES_JGITMENU)->isVisible())
	{
		PPContainer* container = static_cast<PPContainer*>(screen->getControlByID(CONTAINER_LOWRES_JGITMENU));
		GuitarControl* guitarControl = static_cast<GuitarControl*>(container->getControlByID(GUITAR_CONTROL));
	//	updateGuitar |= 
		updateGuitarControl(guitarControl);
	}
	/**/
#else
if (screen->getControlByID(CONTAINER_PIANO_G)->isVisible())
	{
		PPContainer* container = static_cast<PPContainer*>(screen->getControlByID(CONTAINER_PIANO_G));
		PianoControl* pianoControl = static_cast<PianoControl*>(container->getControlByID(PIANO_CONTROL_G));
		updatePiano |= updatePianoControl(pianoControl);
		bool updateG = updateGuitarControl(sectionGuitar->getGuitarControl());
	}	
#endif
