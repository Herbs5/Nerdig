...
void SectionSettings::updateChords()
{
//	for (pp_int32 i = 0; i < GlobalChordConfig::ChordLast; i++)
//		chordDescriptors[i].chordCopy = GlobalChordConfig::getInstance()->getChord((GlobalChordConfig::GlobalChords)i);	
}
...
void SectionSettings::importCurrentChordPalette()
{
	...
	if (panel.runModal() == PPModalDialog::ReturnCodeOK)
	{
		...
		if (exportImport.importChordPalette(pal))
		{
			for (pp_int32 j = 0; j < pal.numChords; j++)
				chordDescriptors[j].chordCopy = pal.chords[j];
			
			currentChord = chordDescriptors[getChordIndex()].chordCopy;
						
			update();
			
			lastChordFile = panel.getFileName();
		}
		else
		{
