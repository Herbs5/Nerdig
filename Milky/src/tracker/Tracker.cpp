	...
	fileSystemChangedListener(NULL)
{
	...
	sectionGuitar = new SectionGuitar(*this);
	sections->add(sectionGuitar);
...
void Tracker::toggleGuitarMode()
{
	...
	PPButton* button = static_cast<PPButton*>(container->getControlByID(BUTTON_GUITAR_M_ALT));
	ASSERT(button);

	switch (pCtrl->chomode)
					{
						case 0: //	ChoChoose: 
						{
							pCtrl->chomode = pCtrl->ChoModify; 
							button->setColor(TrackerConfig::colorPatternEditorBackground);
							button->setTextColor(TrackerConfig::colorRecordModeButtonText);
							button->setText( "??");
							break;
						}
						case 1: //	ChoModify: 
						{
							pCtrl->chomode = pCtrl->ChoPlay; 
							button->setColor(TrackerConfig::colorPatternEditorSelection);
							button->setTextColor(TrackerConfig::colorPatternEditorNote);
...
void Tracker::toggleJgitMenuGuitarSize()
{
	#ifdef __LOWRES__
		PPContainer* container = static_cast<PPContainer*>(screen->getControlByID(CONTAINER_LOWRES_JGITMENU));
		ASSERT(container);
	...
	bool largeGuitar = pCtrl->large;   //(pCtrl->getxScale() == 6 && pCtrl->getyScale() == 3);
	
	PPButton* button = static_cast<PPButton*>(container->getControlByID(BUTTON_JGITMENU_TOGGLESIZE));
	ASSERT(button);
	
	button->setText(largeGuitar ? TrackerConfig::stringButtonCollapsed : TrackerConfig::stringButtonExtended);
	
	if (largeGuitar)
	{
		container->setSize(PPSize(container->getSize().width, container->getSize().height - 24*2));
		container->setLocation(PPPoint(container->getLocation().x, container->getLocation().y + 24*2));
		pCtrl->setLocation(PPPoint(pCtrl->getLocation().x, pCtrl->getLocation().y + 24*2));
	
		pCtrl->setSize(PPSize(screen->getWidth() - 4, 90));
		getPatternEditorControl()->setSize(PPSize(getPatternEditorControl()->getSize().width, getPatternEditorControl()->getSize().height + 24*2));
		pCtrl->large = false;
	}
	else
	{
		...
		pCtrl->setSize(PPSize(screen->getWidth() - 4, 24*5+12));
		getPatternEditorControl()->setSize(PPSize(getPatternEditorControl()->getSize().width, getPatternEditorControl()->getSize().height - 24*2));
		pCtrl->large = true;
	}
...
void Tracker::toggleJamMenuPianoSize()
{
	...
	bool largePiano = (pCtrl->getxScale() == 3 && pCtrl->getyScale() == 3);
...
pp_int32 Tracker::handleEvent(PPObject* sender, PPEvent* event)
{
	...
	else if (event->getID() == eCommand || event->getID() == eCommandRepeat)
	{
		...
		switch (reinterpret_cast<PPControl*>(sender)->getID())
		{
			...
			case BUTTON_GUITAR_VW_M:
			{
				...
				if(pCtrl->vw < 0 )
					pCtrl->vw +=12;
				screen->paintControl(pCtrl, true);
				break;
			}
			...
			case BUTTON_GUITAR_STM_M:
			{
				...
				{
					pCtrl->stemm-=12;
					PPStaticText* staticText = static_cast<PPStaticText*>(container->getControlByID(CONTAINER_GUITAR_INFO8));
					staticText->setIntValue((pp_int32)(pCtrl->stemm/12), 1);
					if (container->isVisible())
						screen->paintControl(container,true);
				}
				break;
			}
			case BUTTON_GUITAR_STM_P:
			{
				#ifdef __LOWRES__
					PPContainer* container = static_cast<PPContainer*>(screen->getControlByID(CONTAINER_LOWRES_JGITMENU));
					ASSERT(container);
			...
			case BUTTON_JGITMENU_NEXT_CHANN:
			{
				#ifdef __LOWRES__
					PPContainer* container = static_cast<PPContainer*>(screen->getControlByID(CONTAINER_LOWRES_JGITMENU));
					ASSERT(container);
				...
				#endif	
				GuitarControl* pCtrl = static_cast<GuitarControl*>(container->getControlByID(GUITAR_CONTROL));
				if(pCtrl->channel <=  moduleEditor->getNumChannels() -7)
				{
					pCtrl->channel++;
					PPStaticText* staticText = static_cast<PPStaticText*>(container->getControlByID(STATICTEXT_JGITMENU_CUR_CHANN));
					staticText->setIntValue((pp_int32)(pCtrl->channel), 1);
					if (container->isVisible())
						screen->paintControl(container,true);
				}
				break;
			}
			case BUTTON_JGITMENU_PREV_CHANN:
			{
				#ifdef __LOWRES__
					PPContainer* container = static_cast<PPContainer*>(screen->getControlByID(CONTAINER_LOWRES_JGITMENU));
					ASSERT(container);
	...
	//		case MAINMENU_EDIT2:
			...
			// instrument editor
			case MAINMENU_GITEDIT:
			{
				if (event->getID() != eCommand)
					break;

				eventKeyDownBinding_InvokeSectionGuitar();
				break;
			}

			case BUTTON_GITEDITOR_EXIT:
			...
			case BUTTON_JGITMENU_TOGGLESIZE:
				if (event->getID() != eCommand)
					break;

				toggleJgitMenuGuitarSize();
				break;
...
void Tracker::ensureSongStopped(bool bResetMainVolume, bool suspend)
{
	...
	updateGuitarControl(sectionGuitar->getGuitarControl());
#ifdef __LOWRES__
	{
...
//		GuitarControl* guitarControl = static_cast<GuitarControl*>(inputContainerCurrent->getControlByID(GUITAR_CONTROL));
		GuitarControl* guitarControl = get_GuitarControl();	
		updateGuitarControl(guitarControl);
	}
#endif
...
void Tracker::selectInstrument(pp_int32 instrument)
{
	...
	pp_int32 weq = moduleEditor->getInstrumentInfo(instrument-1)->weight;

	GuitarControl* guitarControl = get_GuitarControl();
	ASSERT(guitarControl);
	if( weq<= -1)	
	{
		guitarControl->qvw = moduleEditor->analyseInstrument(instrument);
		
	}
	else 
	{
//		printf("\n  ins %i selected  weight= %i", instrument+1, weq);
		guitarControl->qvw = weq;
	}
}
