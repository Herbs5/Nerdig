...
void processSDLUserEvents(const SDL_UserEvent& event)
{
	switch (event.code)
	{
		...
		{
			PPPoint p;
			//p.x = (pp_int32)event.data1;      error on amd64
			//p.x = (__int128)event.data1;    compiles
//			p.x = (pp_int32)event.data1;
//			p.y = (pp_int32)event.data2;
			p.x = (__int128)event.data1;
			p.y = (__int128)event.data2;
...
//			pp_int32 volume = (pp_int32)event.data2;
			pp_int32 note = (__int128)event.data1;
			pp_int32 volume = (__int128)event.data2;
		...
		{
		//	pp_int32 note = (pp_int32)event.data1;
			__int128 note = (__int128)event.data1;
			globalMutex->lock();
			myTracker->sendNoteUp(note);
			globalMutex->unlock();
			break;
...
#endif
{
	...
	bool swapRedBlue = false, fullScreen = false, noSplash = true;  //ddd
