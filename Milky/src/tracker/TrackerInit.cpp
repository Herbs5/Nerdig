...
void Tracker::initUI()
{
	...
	// add sub menu selection buttons (pages)
	{
		...
		pp_int32 dy = (container->getSize().height-2) / 3;
		button = new PPButton(BUTTON_SCOPECONTROL_MUTE, screen, this, PPPoint(container->getLocation().x + 1, container->getLocation().y + 2), PPSize(container->getSize().width/2-3, dy), false, true, false);
		button->setText("Mute");
		button->setColor(TrackerConfig::colorThemeMain);
		button->setTextColor(PPUIConfig::getInstance()->getColor(PPUIConfig::ColorStaticText));
		container->addControl(button);
		
		button = new PPButton(BUTTON_SCOPECONTROL_SOLO, screen, this, PPPoint(container->getLocation().x + 1, container->getLocation().y + 2 + dy), PPSize(container->getSize().width/2-3, dy), false, true, false);
		button->setText("Solo");
		button->setColor(TrackerConfig::colorThemeMain);
		button->setTextColor(PPUIConfig::getInstance()->getColor(PPUIConfig::ColorStaticText));
		container->addControl(button);
		...
		button = new PPButton(MAINMENU_PLAY_SONG, screen, this, PPPoint(container->getLocation().x + container->getSize().width/2, container->getLocation().y + 2), PPSize(container->getSize().width/2-3, dy), false);
		button->setFont(PPFont::getFont(PPFont::FONT_TINY));
		button->setText("Play");
		container->addControl(button);

		button = new PPButton(MAINMENU_STOP, screen, this, PPPoint(container->getLocation().x + container->getSize().width/2, container->getLocation().y + 2 + dy), PPSize(container->getSize().width/2-3, dy), false);
		button->setFont(PPFont::getFont(PPFont::FONT_TINY));
		button->setText("Stop");
		container->addControl(button);
		...
		const char* subMenuTexts[] = {"Main","Song","Ins.","Scopes","Jam","Strings"};	
		const pp_int32 subMenuIDs[] = {BUTTON_0, BUTTON_0+1, BUTTON_0+2, BUTTON_0+3, BUTTON_0+4, BUTTON_0+5};
...
void Tracker::initSectionMainOptions(pp_int32 x, pp_int32 y)
{
	...
	// Setup "Adv" button
	button = static_cast<PPButton*>(container->getControlByID(MAINMENU_ADVEDIT));
	button->setText("AdvE");
#ifndef __LOWRES__	
	button->setSize(PPSize(77>>1, bHeight-1));
	// Add "Git" button
	button = new PPButton(MAINMENU_GITEDIT, screen, this, 
						PPPoint(button->getLocation().x + button->getSize().width+1, button->getLocation().y), 
						PPSize(77>>1, bHeight-1));
	button->setText( "Git" PPSTR_PERIODS);
	container->addControl(button);
#endif	
...
//	static_cast<PPButton*>(container->getControlByID(MAINMENU_GITEDIT))->setText("Git. Ed.");
...
void Tracker::initListboxesSection(pp_int32 x, pp_int32 y)
{
	...
	pp_int32 witz = screen->getWidth() - 4;	
	...
	pp_int32 myDx = 55;
	{
		...
		button = new PPButton(MAINMENU_SMPEDIT, screen, this, PPPoint(x2, y2 + bHeight), PPSize(26, bHeight-1));
		button->setText("smp");		
		container->addControl(button);
...
//   jam 
	{
		pp_int32 height2 = height + 39 + 14;  // 117
		pp_int32 y3 = y - 39 - 14;
		...
		PianoControl* pianoControl = new PianoControl(PIANO_CONTROL, screen, inputControlListener, 
													  PPPoint(container->getLocation().x+2, y2), PPSize(witz, 25*3+12), ModuleEditor::MAX_NOTE); 
		// show C-3
		pianoControl->setBorderColor(TrackerConfig::colorThemeMain);
		pianoControl->setMode(PianoControl::ModePlay);
		pianoControl->setxScale(3);
	...
	/////////////////// guitar
		...
		PPContainer* container = new PPContainer(CONTAINER_LOWRES_JGITMENU, screen, this, PPPoint(0, y3), PPSize(screen->getWidth(),height2), false);
		container->setColor(TrackerConfig::colorThemeMain);

		pp_int32 bHeight = 12;
	//	pp_int32 y2 = y3 ;

		GuitarControl* guitarControl = new GuitarControl(GUITAR_CONTROL, screen, inputControlListener, 
													  PPPoint(container->getLocation().x+2, y3), PPSize(witz, 25*3+15), ModuleEditor::MAX_NOTE); 

//		guitarControl->setBorderColor(TrackerConfig::colorThemeMain);
		guitarControl->setMode(GuitarControl::ModePlay);

		
		container->addControl(guitarControl);
		
		x2 = 0+4;
		pp_int32 y2 =y3 + guitarControl->getSize().height+1;
		
/*dd	*/
		PPStaticText* staticText = new PPStaticText(0, NULL, NULL, PPPoint(x2, y2+2), "Pos", true);
		container->addControl(staticText);	
		
		staticText = new PPStaticText(STATICTEXT_JGITMENU_CURORDER, NULL, NULL, PPPoint(x2 + 3*8+4, y2+2), "FF");
		container->addControl(staticText);	

		button = new PPButton(BUTTON_JGITMENU_NEXTORDERLIST, screen, this, PPPoint(x2 + 5*8+4 + 1, y2), PPSize(12, 11));
		button->setText(TrackerConfig::stringButtonPlus);
		container->addControl(button);
		
		button = new PPButton(BUTTON_JGITMENU_PREVORDERLIST, screen, this, PPPoint(button->getLocation().x + button->getSize().width+1, y2), PPSize(12, 11));
		button->setText(TrackerConfig::stringButtonMinus);
		container->addControl(button);
		...
		staticText = new PPStaticText(STATICTEXT_JGITMENU_CURPATTERN, NULL, NULL, PPPoint(x3 + 3*8+4, y2+2), "FF");
		container->addControl(staticText);	
		...
		staticText = new PPStaticText(0, NULL, NULL, PPPoint(x3, y2+2), "Okt", true);
		container->addControl(staticText);	
//CONTAINER_GUITAR_INFO8
		staticText = new PPStaticText(CONTAINER_GUITAR_INFO8, NULL, NULL, PPPoint(x3 + 3*8+4, y2+2), "5");
		container->addControl(staticText);	

	//	button = new PPButton(BUTTON_JGITMENU_NEXTINSTRUMENT, screen, this, PPPoint(x3 + 5*8+4 + 1, y2), PPSize(12, 11));
		button = new PPButton(BUTTON_GUITAR_STM_P, screen, this, PPPoint(x3 + 5*8+4 + 1, y2), PPSize(12, 11));
		button->setText(TrackerConfig::stringButtonUp);
		container->addControl(button);
		
	//	button = new PPButton(BUTTON_JGITMENU_PREVINSTRUMENT, screen, this, PPPoint(button->getLocation().x + button->getSize().width+1, y2), PPSize(12, 11));
		button = new PPButton(BUTTON_GUITAR_STM_M, screen, this, PPPoint(button->getLocation().x + button->getSize().width+1, y2), PPSize(12, 11));
		button->setText(TrackerConfig::stringButtonDown);
		container->addControl(button);
...
/*
	...
	*/ 
		...
		button = new PPButton(BUTTON_JGITMENU_TOGGLESIZE, screen, this, PPPoint(button->getLocation().x + button->getSize().width, y2+1), PPSize(15, 11), false);
		button->setColor(TrackerConfig::colorThemeMain);
		button->setTextColor(PPUIConfig::getInstance()->getColor(PPUIConfig::ColorStaticText));
		button->setText(TrackerConfig::stringButtonCollapsed);
		container->addControl(button);

		button = new PPButton(BUTTON_GUITAR_PIC, screen, inputControlListener , PPPoint(button->getLocation().x + button->getSize().width+1, y2), PPSize(14, 11));
		button->setText("b");
		container->addControl(button);
		
		x2 = 4;
//		xe = button->getLocation().x + button->getSize().width + 1;
		xe = witz / 2 ;
		xw = (witz - xe)/ 15;
		if(	witz < 600)
			xw--;
		...
		button = new PPButton(MAINMENU_PLAY_SONG, screen, this, PPPoint(x2, y2), PPSize(40, bHeight-1));
		button->setText("Play");		
		container->addControl(button);
		
		x2+=button->getSize().width+1;
		button = new PPButton(MAINMENU_PLAY_PATTERN, screen, this, PPPoint(x2, y2), PPSize((46>>1)-1, bHeight-1));
		button->setText("Pat");		
		container->addControl(button);

		x2+=button->getSize().width+1;
		button = new PPButton(MAINMENU_PLAY_POSITION, screen, this, PPPoint(x2, y2), PPSize((46>>1)+1, bHeight-1));
		button->setText("Pos");
		container->addControl(button);
		
		x2+=button->getSize().width+1;
		button = new PPButton(MAINMENU_STOP, screen, this, PPPoint(x2, y2), PPSize((70>>1), bHeight-1));
		button->setText("Stop");		
		container->addControl(button);
		
		// Add "Edit" button
		x2+=button->getSize().width+1;
		//button = new PPButton(BUTTON_GUITAR_REC, screen, this, PPPoint(x2, y2), PPSize((77>>1), bHeight-1), true, true, false);
		button = new PPButton(MAINMENU_EDIT, screen, this, PPPoint(x2, y2), PPSize((70>>1), bHeight-1), true, true, false);
		button->setText("Rec");
		container->addControl(button);	
		
		x2+=button->getSize().width+1;
		button = new PPButton(BUTTON_GUITAR_VW_M, screen, this, PPPoint(x2, y2), PPSize((40>>1) - 1, bHeight-1));
		button->setText("+");
		container->addControl(button);
		
		x2+=button->getSize().width+1;
		button = new PPButton(BUTTON_GUITAR_VW_P, screen, this, PPPoint(x2, y2), PPSize((40>>1)+1, bHeight-1));
		button->setText("-");
		container->addControl(button);		
		
		
		x2+=button->getSize().width+1;
		staticText = new PPStaticText(0, NULL, NULL, PPPoint(x2 + 2, y2 + 2), "Cha", true);
		container->addControl(staticText);	
		
		staticText = new PPStaticText(STATICTEXT_JGITMENU_CUR_CHANN, NULL, NULL, PPPoint(x2 + 3*8+4, y2+2), "00");
		container->addControl(staticText);	

		button = new PPButton(BUTTON_JGITMENU_PREV_CHANN, screen, this, PPPoint(x2 + 5*8+4 + 1, y2), PPSize(14, 11));
		button->setText(TrackerConfig::stringButtonMinus);
		container->addControl(button);
		
		button = new PPButton(BUTTON_JGITMENU_NEXT_CHANN, screen, this, PPPoint(button->getLocation().x  , y2 - 3 - button->getSize().height), PPSize(14, 14));
		button->setText(TrackerConfig::stringButtonPlus);
		container->addControl(button);
...
////////////////
		...
		button = new PPButton(BUTTON_GUITAR_PAUS, screen, inputControlListener , PPPoint( witz - xw , y2-8), PPSize(xw, 17));
		button->setText("p"); // Pause on 6 strings if cha > 0
		container->addControl(button);
		
		button = new PPButton(BUTTON_GUITAR_DM, screen, inputControlListener , PPPoint( button->getLocation().x - xw +1, y2-8), PPSize(xw, 17));
		button->setText("D-"); // Subdominant of griff
		container->addControl(button);		
		
		button = new PPButton(BUTTON_GUITAR_DP, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("D+"); // Dominant of griff
		container->addControl(button);		
		
		button = new PPButton(BUTTON_GUITAR_BM, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
			button->setColor(TrackerConfig::colorThemeMain);
		button->setText("B-"); //  griff one fret -
		container->addControl(button);		
		
		button = new PPButton(BUTTON_GUITAR_BP, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
			button->setColor(TrackerConfig::colorThemeMain);
		button->setText("B+"); // griff one fret +
		container->addControl(button);		
		
		button = new PPButton(BUTTON_GUITAR_M8, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("8"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M7, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("7"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M6, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("6"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M5, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("5"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M4, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("4"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M3, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("3"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M2, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("2"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M1, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("1"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M_ALT, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setColor(TrackerConfig::colorThemeMain);
		button->setTextColor(PPUIConfig::getInstance()->getColor(PPUIConfig::ColorStaticText));
		button->setText("M"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M_SAVE, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("S"); // 
		container->addControl(button);		

		button = new PPButton(BUTTON_GUITAR_M_LOAD, screen, inputControlListener , PPPoint( button->getLocation().x - xw - 1 , y2-8), PPSize(xw, 17));
		button->setText("L"); // 
		container->addControl(button);		
