...
void SectionSwitcher::showBottomSection(ActiveBottomSections section, bool paint/* = true*/)
{
	switch (bottomSection)
	{
		...
		case ActiveBottomSectionSampleEditor:
			...
			break;
		case ActiveBottomSectionGuitarEditor:
			tracker.sectionGuitar->show(false);
			break;
	}
	...
	switch (bottomSection)
	{
		...
		case ActiveBottomSectionSampleEditor:
			...
			break;
		case ActiveBottomSectionGuitarEditor:
			tracker.sectionGuitar->show(true);
			break;	
...
void SectionSwitcher::showSubMenu(ActiveLowerSectionPages section, bool repaint/* = true*/)
{
	...
	tracker.screen->getControlByID(CONTAINER_LOWRES_JGITMENU)->show(false);
	...
	if ((lastLowerSectionPage == ActiveLowerSectionPageJam && section != ActiveLowerSectionPageJam) 
		|| (lastLowerSectionPage == ActiveLowerSectionPageGuitar && section != ActiveLowerSectionPageGuitar))
		if (patternEditorSize != tracker.getPatternEditorControl()->getSize())
		{
	...
	switch (section)
	{
		...
		case ActiveLowerSectionPageGuitar:
		{
			PPControl* control = tracker.screen->getControlByID(CONTAINER_LOWRES_JGITMENU);
			ASSERT(control);
			patternEditorSize = tracker.getPatternEditorControl()->getSize();
			PPSize size(tracker.screen->getWidth(), control->getLocation().y);
			if (tracker.getPatternEditorControl()->getSize() != size)
				tracker.getPatternEditorControl()->setSize(size);
			tracker.hideInputControl();
			tracker.screen->getControlByID(CONTAINER_LOWRES_JGITMENU)->show(true);
			break;
		}
	}
