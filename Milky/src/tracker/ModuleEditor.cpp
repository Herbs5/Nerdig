...
pp_int32 ModuleEditor::analyseInstrument( pp_int32 inst )
{
	pp_int32 note, scale, wei =  0, i, j, k,  noco = 0;
	pp_int32 pattns = module->header.patnum;
	pp_int64 qweight[12], scal;
	for (pp_int32 qu = 0; qu <=11; qu++)
	{
		qweight[qu]=0;
	}
	for (k = 0; k < pattns; k++)
	{
		...
		for (i = 0; i < pattern->rows; i++)
			for (j = 0; j < pattern->channum; j++)
			{
				...
				if (src[1] == inst)  //src[0]=note  src[1]=instrument index
				{
					note=src[0]-1;
					noco++;
					scale = note - abs(note/12)*12;
		//			printf(":%i ",scale);
					for (pp_int32 qu = 0; qu <=11; qu++)
					{
						scal = scale - qu;  //für c die 5 ist für e die 1
						if (scal <= -1)
							scal += 12;
						qweight[qu] += qcircle[scal]; // 
					}
				}
			}
	...
	// find smalest weight
	if(noco>=  pattns * 3)   //NOteCOunt >= 
	{
		scal =  qweight[0];
		// wei = 0;
		for (pp_int32 qu = 1; qu <=11; qu++)
		{
			if (scal  >= qweight[qu] )
			{
				scal = qweight[qu] ;
	...
	//	printf("\n   Inst nr:%i     weight = %i    qscalesum= %i\n %iNotes %s \n      ", inst, wei, scal, noco, module->instr[inst-1].name);
	//	printf("	sum1:%i s2:%i s3:%i \ns4:%i s5:%i s6:%i \ns9:%i s12:%i \n      ", qweight[0],qweight[1],qweight[2],qweight[3],qweight[4],qweight[5],qweight[8],qweight[11]  );
		getInstrumentInfo(	inst-1)->weight =wei; // erstmal bekannt
		for (pp_int32 qu = 0; qu <=5; qu++)
		{
					printf("\n   Inst nr:%i weight = %i ", qu+1, getInstrumentInfo(qu)->weight);
		}
		//	instruments[inst].weight =wei; 
		return wei;
	}		
	...
	for (k = 0; k < module->header.patnum; k++)
	{
		...
		for (i = 0; i < pattern->rows; i++)
			for (j = 0; j < pattern->channum; j++)
			{
				...
				if (src[1])  //src[0]=note  src[1]=instrument ID
				{
					bitMap[src[1]-1] = TRUE;  //  instrument at src is used
				}
			}
