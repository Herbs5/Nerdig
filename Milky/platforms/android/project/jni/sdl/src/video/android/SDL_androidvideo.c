...
static int sWindowWidth = 1024;
static int sWindowHeight = 768;
static int sNativeWWidth;
static int sNativeWHeight;
...
bool autoResizingActivated = true;
...
int ANDROID_VideoInit( _THIS, SDL_PixelFormat *vformat) {
	...
	SDL_modelist[2]->w = 800;
	SDL_modelist[2]->h = 500;
	SDL_modelist[3]->w = 720;
	SDL_modelist[4]->h = 480; 
	SDL_modelist[4]->w = 640;
	SDL_modelist[4]->h = 480; 
	SDL_modelist[5] = NULL;
...
extern void JAVA_EXPORT_NAME(DemoGLSurfaceView_nativeMouse)(JNIEnv* env, jobject thiz, jint x, jint y, jint action) {
	...
	if (action == MOUSE_DOWN || action == MOUSE_UP)
		SDL_PrivateMouseButton((action == MOUSE_DOWN) ? SDL_PRESSED : SDL_RELEASED, 1, nx, ny);
	if (action == MOUSE_MOVE)
		SDL_PrivateMouseMotion(0, 0, nx, ny);
}
